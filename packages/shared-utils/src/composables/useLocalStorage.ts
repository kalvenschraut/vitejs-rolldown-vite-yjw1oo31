...
export function useLocalStorage<T>(
  ...
  window.addEventListener('storage', (e) => {
    if (e.key === key && e.newValue !== serializer.write(storedValue.value)) {
      try {
        storedValue.value = e.newValue ? serializer.read(e.newValue) : defaultValue as T;
        console.log(`localStorage updated from another tab [${key}]:`, storedValue.value);
      } catch (error) {
...
export function useLocalStorageObject<T extends Record<string, any>>(
  ...
  const [storedValue, setValue, remove] = useLocalStorage(key, { defaultValue });
